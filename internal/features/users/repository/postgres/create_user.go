...
func (r *UsersRepository) CreateUser(ctx context.Context, user *domain.User) (*domain.User, error) {
	...
	defer cancel()
	query := `INSERT INTO todoapp.users (full_name, phone_number) VALUES($1,$2) RETURNING id, version, full_name, phone_number`
	...
	userDomain := domain.NewUser(userModel.ID, userModel.Version, user.FullName, user.PhoneNumber)
